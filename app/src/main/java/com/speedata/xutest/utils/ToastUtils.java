 ...
 * ━━━━━━神兽出没━━━━━━
 *
 * 创   建:Reginer in  2017/2/17 12:02.
 * 联系方式:QQ:282921012
 * 功能描述:吐司相关工具类
 */
public class ToastUtils {
    ...
    private static void showToast(@StringRes int resId, int duration) {
        showToast(AppFid.getInstance().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void showToast(@StringRes int resId, int duration, Object... args) {
        showToast(String.format(AppFid.getInstance().getResources().getString(resId), args), duration);
    }
    ...
    private static void showToast(CharSequence text, int duration) {
        ...
        if (sToast == null) {
            sToast = Toast.makeText(AppFid.getInstance(), text, duration);
        } else {
            sToast.setText(text);
